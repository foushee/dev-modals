---
title: "Epistemic Modals in Children & Adults"
author: "Ruthe Foushee"
date: "Updated 04 April 2021"
output:
  html_document:
    keep_md: yes
    toc: yes
...
```{r data, echo=F}
d <- read.csv('data_anonymized/dev_modals_data.csv')
...
d$modal_pair <- factor(d$modal_pair , levels=c('can-could', 'could-should', 'could-will', 'should-will'))
```
...
Do adults and children typically select the same modal for both trials testing each modal-pair / probability?  

### Adults
```{r consistent-adults-binom, echo=F, message=FALSE, warning=FALSE, paged.print=FALSE}
means.df <- d %>%
  dplyr::select(age_group, modal_pair, proportion, partID, can, could, should, will) %>%
...
adcons.df <- means.df %>%
  ...
  group_by(proportion, modal_pair, modal) %>%
      dplyr::summarize(consistent_yes = sum(ppmean==1),
                       consistent_no = sum(ppmean==0),
                       inconsistent=sum(ppmean==.5), 
                       n=n(),
                       binom_est = binom.test(consistent_yes, n, alternative='two.sided')$estimate,
                       binom_p = binom.test(consistent_yes, n, alternative='two.sided')$p.value) %>%
  filter(consistent_no < 21) %>%
  dplyr::select(-consistent_no)

  kable(adcons.df, caption='Adult Internal Consistency') %>%
  kable_styling('striped', full_width=F) %>%
  kableExtra::row_spec(which(adcons.df$binom_p<.05), background='lightgreen', bold=T)%>%
  collapse_rows(columns = 1:2, valign = "top")%>%
  footnote(general='Via Exact Binomial Test, chance=.5.',
           symbol='Green rows indicate majority of adults provided internally consistent response.')
``` 

### Children  
```{r consistent-kids-binom, echo=F, message=FALSE, warning=FALSE, paged.print=FALSE}
chicons.df <- means.df %>%
  ...
  group_by(proportion, modal_pair, modal) %>%
      dplyr::summarize(consistent_yes = sum(ppmean==1),
                       ...
                       binom_est = binom.test(consistent_yes, n, alternative='two.sided')$estimate,
                       binom_p = binom.test(consistent_yes, n, alternative='two.sided')$p.value) %>%
  filter(consistent_no < 44) %>%
  select(-consistent_no)

  kable(chicons.df, caption='Child Internal Consistency') %>%
  kable_styling('striped', full_width=F) %>%
  row_spec(which(chicons.df$binom_p<.05), background='lightgreen', bold=T)%>%
  collapse_rows(columns = 1:2, valign = "top")%>%
  footnote(general='Via Exact Binomial Test, p=.5.',
           symbol='Green rows indicate majority of children provided internally consistent response.')
``` 
...
d %>%
  ...
  ggplot(., aes(x=modal_pair, y=mean, fill=modal)) +
    ...
    facet_grid(cols=vars(age_year), rows=vars(prob.label), 
               drop=T, scales='free') +
...
ggsave("stacked-barplot-sans_exp2.png", device = "png", 
       path = "plots/",
...
s.df <- d %>%
  select(age, age_group, modal_pair, prob.label, can, could, should, will) %>%
  gather(modal, selection, -age, -modal_pair, -prob.label, -age_group) %>%
  group_by(age_group, modal_pair, prob.label) 
  
s.df$relevant <- 0
s.df$relevant[s.df$modal_pair == 'can-could' & s.df$modal=='can'] <- 1
s.df$relevant[s.df$modal_pair %in% c(
  'can-could', 'could-should', 'could-will') & s.df$modal=='could'] <- 1
s.df$relevant[s.df$modal_pair %in% c(
...
s.df %>%
  ...
  stat_smooth(method='lm') +
    ...
    modals.lineplot.theme +
    scale_color_manual(values=modal.colors,
                      name='Modal') +
  labs(x='Age (years)', y='Mean Selection', 
       title='Epistemic Modal Preference by Age and Outcome Probability')

ggsave("lineplot.pdf", device = "pdf", 
       path = "plots/",
       scale = 1, width = 10, height = 8, units = "in")
```
...
```{r}
mod20 <- glm(adultlike ~ (proportion + age + modal_pair)^2,
               family='binomial', d[d$age_group=='child',])
...
groupc <- mod20$coef[5]
X1.X2 <- mod20$coef[6]
X1.groupb <- mod20$coef[7]
X1.groupc <- mod20$coef[8]
X2.groupb <- mod20$coef[9]
X2.groupc <- mod20$coef[10]

X1_range <- seq(from=0, to=100, by=.01)
...
ggplot(plot.data, aes(x=X1, y=prob, color=group)) + # asking it to set the color by the variable "group" is what makes it draw three different lines
  geom_line(lwd=2) + 
  modals.lineplot.theme +
  labs(x="X1", y="P(outcome)", title="Probability of adult-like modal selection") 
```

### Logit plots

```{r mod20plot}
mod20 <- glm(adultlike ~ age*modal_pair,
               family='binomial', 
...
ggplot(plot.data, aes(x=X1, y=prob, color=group)) + 
  ...
  scale_color_manual(values=c('#FCA636FF','#E16462FF'), labels=c('could-should', 'could-will'), name='Modal Contrast') +
  modals.lineplot.theme.sans +
  theme(legend.position = 'bottom') +
  geom_hline(yintercept=.5) +
  ylim(0, 1) +
  labs(x="Child Age (years)", y="p(Adult-like modal selection)", title="20% Outcome") #: Probability of Adult-like Modal Selection with Age") 

ggsave("mod20prob_exp2.png", device = "png", 
       path = "plots/",
       scale = 1, width = 6, height = 6, units = "in")
```

```{r mod80plot}
mod80 <- glm(adultlike ~ (age+modal_pair)^2,
               family='binomial', 
...
ggplot(plot.data, aes(x=X1, y=prob, color=group)) + 
  geom_line(lwd=1.5, lty=2) + 
  scale_color_manual(values=c('#E16462FF', '#0D0887FF'), labels=c('could-will', 'should-will'), name='Modal Contrast') +
  modals.lineplot.theme.sans +
  theme(legend.position = 'bottom') +
  geom_hline(yintercept=.5) +
  ylim(0, 1) +
...
ggsave("mod80prob_exp2.png", device = "png", 
       path = "plots/",
       scale = 1, width = 6, height = 6, units = "in")
```

```{r mod100plot}
mod100 <- glm(adultlike ~ age*modal_pair,
               family='binomial', 
...
ggsave("mod100prob_exp2.png", device = "png", 
       path = "plots/",
       scale = 1, width = 6, height = 6, units = "in")
```
...
d %>%
  ...
  group_by(proportion, modal_pair, adult_modal, age_year, partID) %>%
      dplyr::summarise(ppmean = na.mean(adultlike)) %>%
  group_by(age_year, proportion, modal_pair, adult_modal) %>%
      dplyr::summarize(n = n(),
            mean = mean(ppmean),
            ciLow = mean - ci.low(ppmean),
            ciHi = mean + ci.high(ppmean)) %>%
  ...
  pack_rows('4-year-olds', 1, 7) %>%
  pack_rows('5-year-olds', 8, 14) %>%
  pack_rows('6-year-olds', 15, 21) 
``` 
...
```{r ad.glmer, echo=F}
ad.glmer <- glmer(adultlike ~ age*as.factor(proportion) + (1|partID), family='binomial', 
...
```{r mod20stronger}
mod20 <- glm(stronger ~ age*modal_pair,
               family='binomial', 
             d[d$proportion==20 & d$age_group=='child', ])
...
ggsave("mod20stronger_exp2.png", device = "png", 
       path = "plots/",
       scale = 1, width = 6, height = 6, units = "in")
```

```{r mod80stronger}
mod80 <- glm(stronger ~ (age+modal_pair)^2,
               family='binomial', 
             d[d$proportion==80 & d$age_group=='child', ])
...
ggsave("mod80stronger_exp2.png", device = "png", 
       path = "plots/",
       scale = 1, width = 6, height = 6, units = "in")
```
...
ggsave("mod100stronger_exp2.png", device = "png", 
       path = "plots/",
       scale = 1, width = 6, height = 6, units = "in")
```
...
d %>%
  filter(age_group =='child',
         !is.na(stronger)) %>%
  group_by(proportion, modal_pair, stronger_modal, age_year, partID) %>%
      dplyr::summarise(ppmean = na.mean(stronger)) %>%
  group_by(age_year, proportion, modal_pair, stronger_modal) %>%
      dplyr::summarize(n = n(),
            mean = mean(ppmean),
            ciLow = mean - ci.low(ppmean),
            ciHi = mean + ci.high(ppmean)) %>%
  ...
  column_spec(1, color='white') %>%
  collapse_rows(columns = 1:4, valign = "top")%>%
  pack_rows('4-year-olds', 1, 8) %>%
  pack_rows('5-year-olds', 9, 16) %>%
  pack_rows('6-year-olds', 17, 23) %>%
  footnote(general='Excludes can-could trials, where no clear stronger modal.')
